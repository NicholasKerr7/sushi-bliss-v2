            ...
            {Array.from({ length: count }, (_, index) => (
              <button
                ...
                className={classNames(
                  "h-1.5 rounded-full transition-all duration-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--sb-gold)]",
                  index === currentIndex
                    ? "w-12 bg-[var(--sb-red-bright)] shadow-[0_0_18px_var(--sb-red-glow)]"
                    : "w-6 bg-white/24 hover:bg-white/46",
                )}
                key={index}
        ...
        <div className="flex items-center gap-3">
          {Array.from({ length: count }, (_, index) => (
            <button
              ...
              aria-label={`${labelPrefix} ${index + 1} of ${count}`}
              className={classNames(
                "h-3 rounded-full transition-all duration-300 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--sb-gold)]",
                index === currentIndex
                  ? "w-8 bg-[var(--sb-red-bright)] shadow-[0_0_18px_var(--sb-red-glow)]"
                  : "w-3 bg-white/26 hover:bg-white/46",
              )}
              key={index}
              onClick={() => onSelect(index)}
              type="button"
