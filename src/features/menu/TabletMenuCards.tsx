    ...
    <article className="relative overflow-hidden rounded-[9px] border border-[var(--sb-border)] bg-black/48">
      ...
      >
        ...
        <div className="p-4">
          ...
          <p className="mt-2 line-clamp-2 min-h-10 text-[14px] leading-5 text-white/72">
      ...
      <button
        ...
        className="absolute bottom-4 right-4 grid h-9 w-9 place-items-center rounded-full border border-[var(--sb-border-strong)] bg-black/55"
...
export function TabletCompactMenuRow({
  ...
  return (
    <article className="grid grid-cols-[120px_1fr_44px] items-center gap-4 rounded-[8px] border border-[var(--sb-border)] bg-black/34 p-2">
      <button
        className="relative h-[64px] overflow-hidden rounded-[6px]"
        onClick={() => onViewDetails(item)}
        type="button"
      >
        <Image
      ...
      <button
        ...
        type="button"
      >
        <h3 className="text-[16px] text-white">{item.name}</h3>
        <p className="mt-1 text-sm text-white/58">{item.description}</p>
        <p className="mt-1 text-[16px] text-[var(--sb-gold)]">
          {formatMoney(item.priceCents)}
        </p>
      </button>
      <button
