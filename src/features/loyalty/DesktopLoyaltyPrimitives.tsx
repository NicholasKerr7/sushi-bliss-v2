  ...
  return (
    <div
      ...
      className={classNames(
        ...
        large ? "h-[152px] w-[152px]" : "h-[132px] w-[132px]",
      )}
      role="img"
    >
    ...
    >
      ...
      <div
        className={classNames(
          ...
          large ? "grid-cols-[152px_1fr]" : "grid-cols-[132px_1fr]",
        )}
      >
        <DesktopMemberQr account={account} large={large} />
        <div>
          <p className="editorial-title text-[22px] uppercase tracking-[0.08em] text-white">
            {mockUser.name}
          </p>
          <p className="mt-1 text-[14px] text-[var(--sb-gold-soft)]">
            Bliss Member
          </p>
          <p className="mt-4 text-[11px] uppercase tracking-[0.1em] text-white/52">
            Member ID
          </p>
  ...
  return (
    <article className="overflow-hidden rounded-[12px] border border-[var(--sb-border)] bg-black/36">
      ...
      <div className="p-4">
        ...
        <h3 className="mt-1 min-h-10 text-[15px] font-semibold leading-5 text-white">
          {reward.title}
        </h3>
        <p className="mt-2 line-clamp-2 min-h-10 text-[12px] leading-5 text-white/56">
          {reward.description}
        </p>
        <button
          className={classNames(
            "mt-4 h-9 w-full rounded-[8px] border text-[11px] uppercase tracking-[0.08em]",
            canRedeem
  ...
  return (
    <div className="divide-y divide-white/10">
      {transactions.slice(0, 4).map((transaction) => (
        <div
          className="grid grid-cols-[1fr_86px_86px] items-center gap-3 py-3 text-[13px]"
          key={transaction.id}
        >
          <div>
    ...
    <div className="grid grid-cols-4 gap-5">
      {desktopLoyaltyPerks.map(([title, copy, icon]) => (
        <article className="text-center" key={title}>
          <span className="mx-auto grid h-12 w-12 place-items-center rounded-full border border-[var(--sb-gold)]/40 bg-black/28">
            <AssetIcon size={24} src={`/assets/icons/${icon}`} />
          </span>
          <p className="mt-3 text-[13px] text-white/82">{title}</p>
