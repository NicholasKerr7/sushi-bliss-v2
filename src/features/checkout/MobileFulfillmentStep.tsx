...
export function FulfillmentStep({
  ...
  return (
    <main className="mt-8 min-[390px]:mt-12">
      <StepHeading
        ...
        {fulfillmentOptions.map((option) => (
          <button
            ...
            className={classNames(
              "grid min-h-[98px] grid-cols-[54px_minmax(0,1fr)_26px] items-center gap-2.5 rounded-[17px] border px-3 text-left transition min-[390px]:min-h-[118px] min-[390px]:grid-cols-[90px_minmax(0,1fr)_38px] min-[390px]:gap-4 min-[390px]:px-4",
            ...
            type="button"
          >
            <span className="grid h-[52px] w-[52px] place-items-center rounded-full border border-[var(--sb-border-strong)] bg-black/22 min-[390px]:h-[78px] min-[390px]:w-[78px]">
              <AssetIcon
                className="h-7 w-7 min-[390px]:h-[42px] min-[390px]:w-[42px]"
                size={32}
                src={option.icon}
              />
            </span>
            <span className="min-w-0">
              <span className="editorial-title block text-[20px] leading-none min-[390px]:text-[27px]">
                {option.label}
              </span>
              <span className="mt-2 block text-[13px] leading-5 text-white/62 min-[390px]:mt-3 min-[390px]:text-[17px] min-[390px]:leading-7">
                {option.description}
              </span>
            </span>
      ...
      <section className="mt-5 grid gap-3">
        <label className="grid min-h-[70px] grid-cols-[38px_minmax(0,1fr)] items-center gap-3 rounded-[15px] border border-[var(--sb-border)] bg-white/[0.025] px-3 transition focus-within:border-[var(--sb-gold)]/58 focus-within:shadow-[0_0_24px_rgba(215,168,79,0.12)] min-[390px]:min-h-[74px] min-[390px]:grid-cols-[48px_1fr] min-[390px]:gap-4 min-[390px]:px-4">
          ...
          <span className="relative block min-w-0">
            ...
            <select
              ...
              className="mt-1 h-9 w-full appearance-none bg-transparent pr-8 text-[15px] font-semibold text-white outline-none min-[390px]:h-10 min-[390px]:text-[18px]"
        ...
        <button
          ...
          className="grid min-h-[70px] grid-cols-[38px_minmax(0,1fr)_24px] items-center gap-3 rounded-[15px] border border-[var(--sb-border)] bg-white/[0.025] px-3 text-left min-[390px]:min-h-[74px] min-[390px]:grid-cols-[48px_1fr_auto] min-[390px]:gap-4 min-[390px]:px-4"
          onClick={() => onSummaryOpenChange(!summaryOpen)}
          type="button"
        >
          <AssetIcon size={28} src={icons.bag} />
          <span className="min-w-0">
            <span className="block text-[11px] uppercase tracking-[0.09em] text-[var(--sb-gold-soft)] min-[390px]:text-[13px] min-[390px]:tracking-[0.13em]">
              View order summary
            </span>
            <span className="mt-1 block text-[14px] text-white/72 min-[390px]:text-[18px]">
...
function SelectionDot({ active }: { active: boolean }) {
  return (
    <span
      aria-hidden="true"
      className={classNames(
        "grid h-6 w-6 place-items-center rounded-full border min-[390px]:h-[30px] min-[390px]:w-[30px]",
        active
    ...
    >
      {active ? (
        <span className="h-3 w-3 rounded-full bg-[var(--sb-red-bright)] min-[390px]:h-[15px] min-[390px]:w-[15px]" />
      ) : null}
    </span>
  );
}
